...
import {
  User, Mail, Phone, Key, Eye, EyeOff, Copy,
  Shield, 
  // TrendingUp,
...
export default function Profile() {
  ...
  /* — Active section tab — */
  const [activeSection, setActiveSection] = useState<"overview" | "security" | "api" | "account">("overview");
  ...
  const handleUpdateApi = () => {
    if (!apiKey || !apiSecret || !apiPassword) { toast.error("All fields required ❌"); return; }
    updateApiMutation.mutate(
      { api_key: apiKey, api_secret: apiSecret, password: apiPassword },
      {
        onSuccess: () => { toast.success("API credentials updated ✅"); setIsEditingApi(false); setApiKey(""); setApiSecret(""); setApiPassword(""); },
        onError:   (err: Error) => { toast.error(err.message || "Failed to update API ❌"); },
      }
    );
  };
  ...
  const handleSendOtp = () => {
    sendOtpMutation.mutate(undefined, {
      onSuccess: () => { toast.success("OTP sent to your email 📩"); setStep("otp"); setResendTimer(60); setCanResend(false); },
      onError:   (err: Error) => { toast.error(err.message || "Failed to send OTP"); },
    });
  };

  const handleChangePassword = () => {
    if (!oldPassword || !newPassword || !confirmPassword) { toast.error("All fields are required ❌"); return; }
      ...
      <main className="flex-1   mx-auto w-full px-4 sm:px-6 lg:px-8 py-6 space-y-6">
          ...
          <div className="relative p-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
              ...
              <div className="flex items-center gap-5">
                <div className="relative flex-shrink-0">
                  <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-gradient-to-br from-red-500 to-red-700 flex items-center justify-center text-white text-2xl sm:text-3xl font-bold shadow-lg">
                    {isLoading ? "?" : (data?.username?.[0]?.toUpperCase() ?? "U")}
                  </div>
              ...
              <div className="flex flex-wrap gap-3">
                <StatPill icon={<CreditCard size={14} />} label="Client ID" value={data?.client_id ?? "—"} />
                <StatPill icon={<Clock size={14} />}       label="Member Since" value="May 2024" />
                <StatPill icon={<Activity size={14} />}   label="Last Login" value="Today" />
              </div>
            </div>
        ...
        <div className="flex gap-1 p-1 bg-muted rounded-xl overflow-x-auto scrollbar-hide">
          {([
            ...
            { key: "account",  label: "Account",    icon: <Building2 size={15} />     },
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <SectionCard title="Profile Details" icon={<User size={16} />}>
              <div className="grid grid-cols-2 gap-4">
                <Field label="Full Name"  value={data?.username} />
                <Field label="Email"      value={data?.email} />
                <Field label="Mobile"     value={data?.phone} />
                <Field label="PAN"        value="DZX****41M" />
                <Field label="UCC"        value={data?.client_id} copy />
            ...
            <SectionCard title="Subscription" icon={<CreditCard size={16} />}>
              <div className="flex items-center justify-between p-4 rounded-xl bg-gradient-to-r from-red-500/10 to-red-400/5 border border-red-500/20">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-0.5">Renews on 31 May 2026</p>
                </div>
                <span className="px-3 py-1 rounded-full bg-green-500/10 text-green-500 text-xs font-semibold">Active</span>
              </div>
              <div className="grid grid-cols-3 gap-3 mt-3">
                {[
                  ...
                  <div key={f.label} className="rounded-xl bg-muted px-3 py-3 text-center">
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch">
    ...
    {/* BROKER DETAILS */}
    <SectionCard title="Broker Details" icon={<Building2 size={16} />} className="min-w-[320px]">
      <div className="grid grid-cols-2 gap-4">
    ...
    {/* LINKED BANK */}
    <SectionCard title="Linked Bank" icon={<CreditCard size={16} />} className="min-w-[320px]">
              ...
              {!isEditingApi ? (
                <div className="space-y-3">
                  ...
                  <SecretRow
                    ...
                    onCopy={() => copyText(data?.api_secret)}
                  />
                  ...
                  <div className="flex items-center gap-2 mt-4 p-3 rounded-xl bg-yellow-500/10 border border-yellow-500/20">
                    ...
                    <p className="text-xs text-yellow-600 dark:text-yellow-400">
                      Never share your API credentials. Rotate them immediately if compromised.
                    </p>
                  </div>
                ...
                <div className="space-y-3">
                  <p className="text-sm text-muted-foreground">Enter your new credentials. Your account password is required to confirm.</p>

                  <div>
                    <label className="text-xs text-muted-foreground mb-1 block">New API Key</label>
                    <input className="wizard-input "
                      placeholder="api_key_xxxxxxxxxxxx" value={apiKey} onChange={e => setApiKey(e.target.value)} />
                  </div>
                  <div>
                    <label className="text-xs text-muted-foreground mb-1 block">New API Secret</label>
                    <input className="wizard-input "
                      placeholder="api_secret_xxxxxxxxxxxx" value={apiSecret} onChange={e => setApiSecret(e.target.value)} />
                  </div>
                  <div>
                    {/* w-full px-3 py-2.5 rounded-xl border border-border bg-muted text-sm focus:outline-none focus:ring-2 focus:ring-red-500 */}
                    <label className="text-xs text-muted-foreground mb-1 block">Account Password</label>
                    <input type="password" className="wizard-input "
                      placeholder="Your account password" value={apiPassword} onChange={e => setApiPassword(e.target.value)} />
                  </div>

                  <div className="flex gap-2 pt-1">
                    <button onClick={handleUpdateApi} disabled={updateApiMutation.isPending}
                    ...
                    <button onClick={() => { setIsEditingApi(false); setApiKey(""); setApiSecret(""); setApiPassword(""); }}
        ...
        {activeSection === "security" && (
          <div className=" mx-auto space-y-4">
            ...
            <SectionCard title="Change Password" icon={<Shield size={16} />}>
              ...
              {step === "password" && (
                <div className="space-y-3">
                  <PasswordField value={oldPassword}     onChange={setOldPassword}     placeholder="Current Password"  show={showOld}     setShow={setShowOld}     label="Current Password"  />
                  <PasswordField value={newPassword}     onChange={setNewPassword}     placeholder="New Password"      show={showNew}     setShow={setShowNew}     label="New Password"      />
                  <PasswordField value={confirmPassword} onChange={setConfirmPassword} placeholder="Confirm Password"  show={showConfirm} setShow={setShowConfirm} label="Confirm Password"  />
                  ...
                  <button onClick={handleSendOtp} disabled={sendOtpMutation.isPending}
                    ...
                    {sendOtpMutation.isPending ? "Sending OTP…" : "Change via OTP instead"}
                  </button>
                </div>
              )}
              ...
              {step === "otp" && (
                <div className="space-y-3">
                  ...
                  <div>
                    <label className="text-xs text-muted-foreground mb-1 block">One-Time Password</label>
                    <input className="wizard-input"
                  ...
                  <button onClick={handleVerifyOtp} disabled={!otp || otp.length < 6 || verifyOtpMutation.isPending}
                    className="w-full py-2.5 rounded-xl bg-red-600 hover:bg-red-700 text-white text-sm font-semibold disabled:opacity-50 transition">
                  ...
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <button onClick={handleSendOtp} disabled={!canResend || sendOtpMutation.isPending}
  ...
  return (
    <div className="space-y-1">
      ...
      <div className="flex items-center gap-2">
        ...
        {copy && value && (
          <button onClick={() => copyText(value)} className="text-muted-foreground hover:text-foreground transition">
            <Copy size={12} />
          </button>
        )}
      </div>
...
function SecretRow({ label, value, show, onToggle, onCopy }: {
  ...
  return (
    <div className="flex items-center justify-between bg-muted px-4 py-3 rounded-xl border border-border gap-3">
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        <button onClick={onCopy}   className="text-muted-foreground hover:text-foreground transition"><Copy   size={14} /></button>
...
function PasswordField({ value, onChange, placeholder, show, setShow, label }: PasswordFieldProps) {
  return (
    <div>
...
function StrengthBars({ isStrong, isPrefixMatch, confirmPassword, newPassword }: {
  isStrong: boolean; isPrefixMatch: boolean;
  ...
  const bars = [
    strength >= 1 ? (strength === 1 ? "bg-red-500" : strength === 2 ? "bg-yellow-500" : "bg-green-500") : "bg-muted",
    strength >= 2 ? (strength === 2 ? "bg-yellow-500" : "bg-green-500") : "bg-muted",
  ...
  return (
    <div className="space-y-2">
      <div className="flex gap-1">
        {bars.map((b, i) => <div key={i} className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${b}`} />)}
      </div>
      <div className="flex justify-between text-xs">
        <span className="text-muted-foreground">
          {strength > 0 && <span className={strength <= 1 ? "text-red-500" : strength === 2 ? "text-yellow-500" : "text-green-500"}>
            {strengthLabel}
          </span>}
