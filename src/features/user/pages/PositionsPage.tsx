...
import { Briefcase, IndianRupee, TrendingUp, Wallet } from "lucide-react";
...
import type { OpenPosition } from "../api/getMarketData";
...
export default function PositionsPage() {
  ...
  const filteredPositions = useMemo(() => {
    ...
    if (filters.search.trim()) {
      ...
      rows = rows.filter(
        (position) =>
          position.tradingSymbol?.toLowerCase().includes(query) ||
          position.securityId?.toString().includes(query),
      );
    }
  ...
  const realizedPnL = positions.reduce(
    (sum, position) => sum + position.realizedProfit,
    0,
  );
  ...
  const columns: Column<OpenPosition>[] = [
    {
      ...
      render: (position) => (
        <span className="font-semibold">{position.tradingSymbol}</span>
      ),
    },
    ...
    <>
      ...
      >
        <Card
          ...
          icon={<Wallet size={20} />}
          color={realizedPnL >= 0 ? "green" : "orange"}
        />
        ...
        <Card
          ...
          color={unrealizedPnL >= 0 ? "green" : "orange"}
        />
