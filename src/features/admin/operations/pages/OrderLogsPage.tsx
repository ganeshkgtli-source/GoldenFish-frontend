...
export default function OrderLogPage() {
  ...
  useEffect(() => {
    if (dateMode === "single") setToDate(fromDate);
  }, [dateMode, fromDate]);
  ...
  useEffect(() => {
    setPage(1);
  }, [clientFilter, statusFilter, exchangeFilter, typeFilter, search, fromDate, toDate]);
...
<main className="p-4 md:p-6 space-y-5 flex-1">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Filter label="All Exchanges" value={exchangeFilter} options={exchangeOptions} onChange={setExchangeFilter} />
...
<div className="rounded-xl border border-border overflow-hidden">
  <div className="overflow-x-auto max-h-[540px]">
    <table className="w-full text-sm min-w-[900px]">
      ...
      <thead className="bg-muted text-muted-foreground sticky top-0 z-10">
        <tr>
          {[
            "Date & Time", "Client Name", "Client ID", "Symbol",
            "Exchange", "Type", "Expiry", "Status", "PnL / Lot", "Total PnL",
          ].map((h) => (
            <th
              key={h}
        ...
        {orders.length === 0 ? (
          ...
          orders.map((row, i) => (
            ...
            >
              ...
              <td className={`px-4 py-3 font-semibold ${row.type === "BUY" ? "text-green-500" : "text-red-500"}`}>
                {row.type}
              </td>
              ...
              <td className="px-4 py-3">
                <div className="flex items-center gap-1.5">
                  {row.status === "Completed" && <CheckCircle size={14} className="text-green-500" />}
                  {row.status === "Rejected" && <XCircle size={14} className="text-red-500" />}
                  {row.status === "Pending" && <Clock size={14} className="text-yellow-500" />}
                  <span className={`text-xs font-medium ${statusStyle[row.status] ?? ""}`}>
                    {row.status}
                  </span>
                </div>
              </td>

              <td className={`px-4 py-3 font-medium ${row.pnlLot >= 0 ? "text-green-500" : "text-red-500"}`}>
                ₹{row.pnlLot}
              </td>

              <td className={`px-4 py-3 font-medium ${row.totalPnl >= 0 ? "text-green-500" : "text-red-500"}`}>
                ₹{row.totalPnl}
              </td>
            </tr>
          ))
  ...
  <div className="flex items-center gap-1 bg-muted px-2 py-1 rounded-lg">
    ...
    {getPaginationRange(page, totalPages).map((p, idx) =>
      p === "..." ? (
        <span key={idx} className="px-2 py-1 text-xs text-muted-foreground">
          …
        </span>
      ) : (
        <button
