...
const allErrors = Array.from({ length: 87 }, (_, i) => ({
  ...
  message:
    i % 2 === 0 ? "Order rejected by broker" : "Slippage exceeded threshold",
...
export default function ErrorLogPage() {
  ...
  const errors = filtered.slice((page - 1) * pageSize, page * pageSize);
          ...
          <div className="flex gap-1 bg-muted px-2 py-1 rounded-lg">
            ...
            {Array.from({ length: totalPages }, (_, i) => (
              <button
                ...
                onClick={() => setPage(i + 1)}
                className={
                  page === i + 1 ? "text-white bg-red-600 px-2" : "px-2"
                }
              >
                {i + 1}
              </button>
